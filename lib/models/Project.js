...
export default class Project {
  ...
  static async getAll(query = {}) {
    try {
      const collection = await getCollection('projects')
      const projects = await collection.find(query).sort({ order: 1, createdAt: -1 }).toArray()
  ...
  static async getPublished() {
    try {
      const collection = await getCollection('projects')
      const projects = await collection.find({ isPublished: true }).sort({ order: 1, createdAt: -1 }).toArray()
      return { success: true, projects }
    } catch (error) {
